antimage_mage_shield = class({})

LinkLuaModifier("modifier_antimage_mage_shield_active", "abilities/antimage/antimage_mage_shield.lua", LUA_MODIFIER_MOTION_NONE)

function antimage_mage_shield:Precache(context)
    PrecacheResource("particle", "particles/units/heroes/hero_antimage/antimage_spellshield_reflect.vpcf", context)
end

function antimage_mage_shield:GetIntrinsicModifierName()
    return nil
end

function antimage_mage_shield:OnUpgrade()
    if not IsServer() then return end

    local caster = self:GetCaster()
    local linked = caster:FindAbilityByName("antimage_physical_shield")
    if linked and linked:GetLevel() ~= self:GetLevel() then
        linked:SetLevel(self:GetLevel())
    end
end

function antimage_mage_shield:CastFilterResult()
    if IsServer() then
        local caster = self:GetCaster()
...
function antimage_mage_shield:GetCustomCastError()
    return "#dota_hud_error_cant_cast_when_stunned"
end

function antimage_mage_shield:OnToggle()
    if not IsServer() then return end
    ...
    if self:GetToggleState() then
        ...
        local other = caster:FindAbilityByName("antimage_physical_shield")
        if other then
        ...
        caster:AddNewModifier(caster, self, "modifier_antimage_mage_shield_active", {})
    else
        ...
        caster:RemoveModifierByName("modifier_antimage_mage_shield_active")
...
modifier_antimage_mage_shield_active = class({})

function modifier_antimage_mage_shield_active:IsPurgable() return false end

function modifier_antimage_mage_shield_active:OnCreated()
    if not IsServer() then return end
...
function modifier_antimage_mage_shield_active:OnIntervalThink()
...
function modifier_antimage_mage_shield_active:DeclareFunctions()
    return {
        MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS
    }
end

function modifier_antimage_mage_shield_active:GetModifierMagicalResistanceBonus()
